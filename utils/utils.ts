export function getTeamBadge(team: string) {
  return (globalThis as any).chrome.runtime.getURL(getTeamBadgeLocalPath(team));
}

function getTeamBadgeLocalPath(team: string) {
  // Premier
  switch (team) {
    case 'Aston':
    case 'Aston Villa':
